...
| メソッド                                 | 説明                                                                                                             |
| :--------------------------------------- | :--------------------------------------------------------------------------------------------------------------- |
| `map(Function<T, R> mapper)`             | 各要素を、指定された関数（`mapper`）または、ラムダ式を使用して**別の型や値に変換**します。                       |
...
| メソッド                                            | 説明                                                                                                                                       |
| :-------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------- |
| `collect(Collector<T, A, R> collector)`             | 処理結果を**`List`、`Set`、`Map`**などの**新しいコレクションに変換・収集**します。 (`Collectors.toList()`, `Collectors.toMap()`などを使用) |
| `toList()`                                          | (Java 16以降) 処理結果を**不変の`List`**に変換します。                                                                                     |
| `toArray()`                                         | 要素を**配列**に変換し、配列型として返します。                                                                                             |
| `reduce(T identity, BinaryOperator<T> accumulator)` | ストリームの要素を**結合**（ギュッと1つにする）し、その単一の結果を返す。                                                                  |
...
| `anyMatch(Predicate<T> predicate)`  | **いずれかの要素**が条件に合致するかどうかを返します（`boolean`）。Map型のcontainsKeyと同じ機能。 |
...
| メソッド                             | 説明                                                                                           |
| :----------------------------------- | :--------------------------------------------------------------------------------------------- |
| `forEach(Consumer<T> action)`        | ストリーム内の各要素に対して**指定された操作を実行**します（戻り値なし）。ラムダ式を使用する。 |
