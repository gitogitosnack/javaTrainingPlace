ここは、私が学習したJavaの知識を整理しまとめておくための場所とする。

## 目次

### Java Standard API / コレクション関連

- [List / Map / Set](Collection_class/what_is_Collection.md)
  - [メソッド一覧](Collection_class/Collection_method_list.md)
- [Optional クラス](Optional_class/what_is_Optional.md)
  - [メソッド一覧](Optional_class/Optional_method_list.md)v
- [String / StringBuilder](String_class/what_is_String.md)
  - [メソッド一覧](String_class/String_method_list.md)
- [日付・時間 API（java.time）](DateTime_class/what_is_DateTime.md)
  - [メソッド一覧](DateTime_class/DateTime_method_list.md)
- [例外処理（Exception Handling）](Exception_handling/what_is_Exception.md)
  - [メソッド一覧](Exception_handling/Exception_method_list.md)
- [Array クラス](Array_class/what_is_Array.md)
  - [メソッド一覧](Array_class/Array_method_list.md)
- [Buffer クラス](Buffer_class/what_is_Buffer.md)
  - [メソッド一覧](Buffer_class/Buffer_method_list.md)
- [Builder クラス](Builder_class/what_is_Builder.md)
  - [メソッド一覧](Builder_class/Builder_method_list.md)
- [File / Files クラス](File_Files_class/what_is_File_Files.md)
  - [メソッド一覧](File_Files_class/File_Files_method_list.md)
- [Stream API](streamAPI/what_is_StreamAPI.md)
  - [メソッド一覧](streamAPI/StreamAPI_method_list.md)
  - [練習コード](streamAPI/StreamApiPractice.java)

### Spring Framework 核心概念（DI / AOP / データアクセス）

- [DI（依存性注入）とアノテーション / Beanスコープ・ライフサイクル](SpringDI/what_is_SpringDI.md)
  - [アノテーション一覧](SpringDI/SpringDI_method_list.md)
- [Spring AOP（アスペクト指向プログラミング）](SpringAOP/what_is_SpringAOP.md)
  - [アノテーション一覧](SpringAOP/SpringAOP_method_list.md)
- [Spring Data JPA / O/Rマッパー](SpringDataJPA/what_is_SpringDataJPA.md)
  - [アノテーション・メソッド一覧](SpringDataJPA/SpringDataJPA_method_list.md)
- [Spring Batch](SpringBatch/what_is_SpringBatch.md)
  - [メソッド一覧](SpringBatch/SpringBatch_method_list.md)
- [Spring Security](SpringSecurity/what_is_SpringSecurity.md)
  - [メソッド一覧](SpringSecurity/SpringSecurity_method_list.md)

### オブジェクト指向・デザインパターン・ボイラープレート削除

- [Lombok](Lombok/what_is_Lombok.md)
  - [アノテーション一覧](Lombok/Lombok_method_list.md)
- [GoF デザインパターン（実務頻出）](DesignPatterns/what_is_DesignPatterns.md)
  - [構造・コード例一覧](DesignPatterns/DesignPatterns_method_list.md)
- [record](Record_class/what_is_Record.md)
  - [構文一覧](Record_class/Record_method_list.md)
- [switch式・パターンマッチング](Switch_class/what_is_Switch.md)
  - [構文一覧](Switch_class/Switch_method_list.md)
- [ラムダ式・匿名クラス](Lambda_class/what_is_Lambda.md)
  - [構文・インターフェース一覧](Lambda_class/Lambda_method_list.md)

### アプリケーション共通処理・連携基盤

- [JSON / XML パースライブラリ（Jackson/Gson）](JsonXml/what_is_JsonXml.md)
  - [アノテーション・メソッド一覧](JsonXml/JsonXml_method_list.md)
- [HTTPクライアント処理](HttpClient_class/what_is_HttpClient.md)
  - [メソッド一覧](HttpClient_class/HttpClient_method_list.md)
- [ログライブラリ（SLF4J/Logback）](Logging/what_is_Logging.md)
  - [メソッド一覧](Logging/Logging_method_list.md)
- [Apache POI](ApachePOI/what_is_ApachePOI.md)
  - [メソッド一覧](ApachePOI/ApachePOI_method_list.md)

### バリデーション（入力チェック）

- [Jakarta Validation（Bean Validation）](Validation/what_is_Validation.md)
  - [アノテーション一覧](Validation/Validation_method_list.md)

### その他

- [for文](for/Main1.java)
  - [continue文とは](for/what_is_Continue.md)
- [クラス連携の練習](linkTwoClasses)
  - [CalcLogics.java](linkTwoClasses/CalcLogics.java)
  - [MainCalc.java](linkTwoClasses/MainCalc.java)
- [Markdownファイルの書き方](HowToWriteMarkdownFile.md)
